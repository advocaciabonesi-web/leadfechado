...
function Btn({ children, onClick, disabled, variant = 'primary', style = {} }) {
  ...
  const v = {
    primary: {
      background: disabled
        ? '#1a1a28'
        : `linear-gradient(135deg,${T.gold},#9a6818)`,
      color: disabled ? T.textDim : '#0a0a14',
    },
    ghost: {
...
function CopyBtn({ text }) {
  ...
  return (
    <button
      ...
      style={{
        background: ok ? T.greenBg : 'transparent',
        border: `1px solid ${ok ? T.green : T.border}`,
        color: ok ? T.green : T.textMuted,
        borderRadius: 7,
        padding: '4px 12px',
        fontSize: 11,
        cursor: 'pointer',
...
const P_MENSAGEM = (
  abordagem,
  instrucao
) => `Você é um advogado trabalhista gerando mensagens de WhatsApp para FECHAR CONTRATO.
...
const ABORDAGENS = [
  {
    ...
    label: 'Urgência',
    cor: T.red,
    desc: 'Lead procrastinando — prescrição em risco',
  },
  {
    id: 'empatia',
    icon: '🤝',
    label: 'Empatia',
    cor: T.purple,
    desc: 'Lead abalado, inseguro, com medo',
  },
  {
    id: 'autoridade',
    icon: '⚖️',
    label: 'Autoridade',
    cor: T.gold,
    desc: 'Lead quer sentir que fala com expert',
  },
  {
    id: 'racional',
    icon: '📊',
    label: 'Racional',
    cor: T.blue,
    desc: 'Lead quer entender antes de decidir',
  },
  {
    id: 'exclusivo',
    icon: '🎯',
    label: 'Exclusividade',
    cor: T.green,
    desc: 'Lead precisa do empurrão final',
  },
];
const INSTRUCOES = {
  urgencia:
    'Mencione DIRETAMENTE o prazo prescricional de 2 anos após extinção do contrato (art. 7º XXIX CF/88, art. 11 CLT). Cada dia sem agir é risco real de perder direitos. Tom firme e respeitoso.',
  empatia:
    'Valide o sofrimento primeiro. Mostre que entende a situação difícil. O advogado é parceiro, não vendedor. Tom acolhedor, humano. Cite proteção legal que ampara essa pessoa.',
  autoridade:
    'Demonstre domínio técnico imediato. Cite artigos da CLT e Súmulas do TST aplicáveis ao caso. Tom seguro, de referência no assunto. O cliente deve sentir que está com o melhor.',
  racional:
    'Apresente os direitos de forma objetiva. Relate o que ele tem a ganhar e o risco de não agir. Cite o percentual de êxito da área. Tom educativo, transparente, sem pressão.',
  exclusivo:
    'Crie senso de oportunidade — atenção personalizada, análise completa feita, janela aberta agora. Tom de oportunidade única sem desespero. Valorize o tempo que já dedicou ao caso.',
};
...
function ToolLead() {
  ...
  const analisar = async () => {
    if (txt.trim().length < 15) {
      setErr('Descreva melhor o caso.');
      return;
    }
    setErr('');
    setLoading(true);
    try {
      const r = await callClaude(P_ANALISE, txt);
      setAnalysis(r);
      setStep(2);
    } catch (e) {
      setErr(e.message);
  ...
  const gerarMsgs = async (abId) => {
    ...
    try {
      ...
      const r = await callClaude(
        P_MENSAGEM(ABORDAGENS.find((a) => a.id === id)?.label, INSTRUCOES[id]),
        ctx
      );
      setMsgs({ ...r, abId: id });
      setStep(3);
  ...
  if (step === 1)
    return (
      <div>
        <Title sub="Cole o relato em qualquer formato — WhatsApp, formulário, anotação verbal, e-mail.">
          ⚡ Analisar Lead
        </Title>
        <textarea
          value={txt}
          ...
          rows={9}
          placeholder="Cole aqui o relato do lead trabalhista...&#10;&#10;Ex: 'Fui demitido depois de 6 anos sem receber horas extras e com salário abaixo do piso da categoria. A empresa disse que foi sem justa causa mas não pagou tudo.'"
          style={{
            ...inp,
            resize: 'vertical',
            lineHeight: 1.7,
        ...
        >
          {loading
            ? '⏳ Analisando com base jurídica completa...'
            : '→ Analisar Lead'}
        </Btn>
      </div>
    );
    ...
    return (
      <div>
        ...
        >
          ...
          <Title sub="Análise jurídica completa com base na CLT, TST e CF/88.">
            Análise do Caso
          </Title>
        </div>
        ...
        <Card>
          ...
          >
            {[
              ...
              ['Tempo', analysis.tempo_empresa],
              ['Salário est.', analysis.salario_estimado],
            ].map(([k, v]) => (
              <div key={k}>
                <Lbl>{k}</Lbl>
          ...
          <div
            style={{
              ...
              gap: 10,
              padding: '10px 14px',
              background: NB[nv] || T.surface,
              borderRadius: 8,
              marginBottom: 14,
            }}
          >
            <Tag color={NC[nv] || T.textMuted} bg={NB[nv]}>
              {(analysis.nivel || '').toUpperCase()}
            </Tag>
            <span
              style={{
                color: NC[nv] || T.textMuted,
                fontSize: 13,
                fontStyle: 'italic',
              }}
            >
          ...
          >
            {(analysis.violacoes || []).map((v, i) => (
              <Tag key={i} color={T.gold}>
                {v}
              </Tag>
            ))}
          </div>
          ...
          >
            <div>
              ...
              {(analysis.provas_mencionadas || []).map((p, i) => (
                <div
                  key={i}
                  style={{ color: T.green, fontSize: 12, padding: '2px 0' }}
                >
                  ✓ {p}
                </div>
              ))}
              {!(analysis.provas_mencionadas || []).length && (
            ...
            <div>
              ...
              {(analysis.provas_sugeridas || []).map((p, i) => (
                <div
                  key={i}
                  style={{ color: T.yellow, fontSize: 12, padding: '2px 0' }}
                >
                  → {p}
                </div>
              ))}
        ...
        {analysis.prescricao_alerta &&
          analysis.prescricao_alerta !== 'null' && (
            <InfoBox color={T.red}>
              ⏰ <strong>Prescrição:</strong> {analysis.prescricao_alerta}
            </InfoBox>
          )}
        {analysis.alerta_risco && analysis.alerta_risco !== 'null' && (
          <InfoBox color={T.yellow}>
            ⚠ <strong>Risco:</strong> {analysis.alerta_risco}
          </InfoBox>
        )}
        ...
        <Card style={{ background: T.surface }}>
          ...
          >
            Cada abordagem gera mensagens completamente diferentes. Escolha com
            base no perfil do lead.
          </p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
            {ABORDAGENS.map((a) => (
              <button
                ...
                onClick={() => setAb(a.id)}
                style={{
                  background: ab === a.id ? `${a.cor}12` : T.card,
                  border: `1px solid ${ab === a.id ? a.cor : T.border}`,
                  borderRadius: 9,
                  padding: '11px 14px',
                  cursor: 'pointer',
                  textAlign: 'left',
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <span
                    style={{ color: T.textMuted, fontSize: 11, marginLeft: 8 }}
                  >
                    {a.desc}
                  </span>
                </div>
        ...
        >
          {loading
            ? '⏳ Gerando mensagens...'
            : `→ Gerar com Abordagem ${
                ABORDAGENS.find((a) => a.id === ab)?.icon
              } ${ABORDAGENS.find((a) => a.id === ab)?.label}`}
        </Btn>
      </div>
    );
  }
  ...
  if (step === 3 && msgs) {
    ...
    return (
      <div>
        <Title sub="Copie e cole direto no WhatsApp.">
          Mensagens Prontas ✓
        </Title>
        {abObj && (
          <InfoBox color={abObj.cor}>
            {abObj.icon} Abordagem <strong>{abObj.label}</strong> — {abObj.desc}
          </InfoBox>
        )}

        {[
          ['MENSAGEM PRINCIPAL', 'mensagem_principal', T.gold],
          ['FOLLOW-UP 24H', 'followup_24h', T.purple],
        ].map(([lbl, key, cor]) => (
        ...
        <Card style={{ background: T.surface }}>
          ...
          <div
            style={{ display: 'flex', flexWrap: 'wrap', gap: 7, marginTop: 8 }}
          >
            {ABORDAGENS.filter((a) => a.id !== msgs.abId).map((a) => (
              <button
                key={a.id}
                onClick={() => gerarMsgs(a.id)}
                disabled={loading}
                style={{
                  background: T.card,
                  border: `1px solid ${T.border}`,
    ...
    <div>
      ...
      <Card>
        ...
        <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
          {STAGES.map((s) => (
            <button
              key={s.id}
              onClick={() => up('stage', s.id)}
              style={{
                background: f.stage === s.id ? `${s.cor}18` : 'transparent',
                border: `1px solid ${f.stage === s.id ? s.cor : T.border}`,
                borderRadius: 7,
                padding: '5px 12px',
                color: f.stage === s.id ? s.cor : T.textMuted,
                fontSize: 12,
                cursor: 'pointer',
                transition: 'all 0.15s',
              }}
      ...
      <Card>
        ...
        >
          {[
            ...
            <div
              key={k}
              style={{ gridColumn: k === 'endereco' ? '1/-1' : 'auto' }}
            >
              <Lbl>{ph}</Lbl>
              <input
                value={f[k] || ''}
      ...
      <Card>
        ...
        >
          ...
          {[
            ['violacoes', 'Violações identificadas'],
      ...
      <Card>
        ...
        >
          <div>
            <Lbl>Valor fixo (R$)</Lbl>
            <input
              value={f.honorarios || ''}
              onChange={(e) => up('honorarios', e.target.value)}
              placeholder="ex: 2000"
              type="number"
              style={inp}
            />
      ...
      <Card>
          ...
          {DOCS.map((d) => {
            ...
            return (
              <button
                ...
                onClick={() => toggleDoc(d)}
                style={{
                  background: ok ? T.greenBg : T.surface,
                  border: `1px solid ${ok ? T.green : T.border}`,
                  borderRadius: 7,
                  padding: '5px 11px',
                  color: ok ? T.green : T.textMuted,
        ...
        >
          {(f.docs || []).length}/{DOCS.length} documentos recebidos
        </div>
      </Card>
      ...
      <Card>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
          <input
            ...
            placeholder="Anotação... (Enter para salvar)"
            style={{ ...inp, flex: 1 }}
          />
          <Btn
            onClick={addNota}
            style={{ padding: '8px 14px', whiteSpace: 'nowrap', fontSize: 12 }}
          >
            + Anotar
          </Btn>
        </div>
        {!(f.hist || []).length && (
          <div
            style={{ color: T.textDim, fontSize: 12, fontFamily: 'monospace' }}
          >
      ...
      <Btn onClick={() => onSave(f)} style={{ width: '100%', padding: 13 }}>
        💾 Salvar Ficha
      </Btn>
    </div>
  );
...
function ToolCRM() {
  ...
  const save = (nl) => {
    setLeads(nl);
    window.storage?.set('lf_leads', JSON.stringify(nl)).catch(() => {});
  };
  const addLead = () => {
    ...
    setForm({ nome: '', contato: '', caso: '', stage: 'novo' });
    setShowForm(false);
  };
  const saveFicha = (f) => {
    save(leads.map((l) => (l.id === f.id ? f : l)));
    setFicha(f);
  };
  const delLead = (id) => {
  ...
  return (
    <div>
      ...
      >
        <Title sub="Pipeline completo do primeiro contato ao contrato assinado.">
          📋 CRM de Leads
        </Title>
        <Btn
      ...
      {!leads.length && !showForm && (
        ...
        >
          ...
          >
            Cadastre o primeiro lead para iniciar o pipeline.
          </div>
          <Btn
      ...
      {leads.length > 0 && (
        <InfoBox color={T.gold}>
          💡 Clique em qualquer lead para abrir a ficha completa com documentos,
          histórico e honorários.
        </InfoBox>
      )}
          ...
          >
            {[
              ...
              <input
                ...
                placeholder={ph}
                style={{ ...inp, gridColumn: k === 'caso' ? '1/-1' : 'auto' }}
              />
            ))}
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <Btn onClick={addLead} style={{ flex: 1 }}>
              Adicionar
            </Btn>
              ...
              >
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
                    ...
                    >
                      {l.contato} · {l.createdAt}
                    </span>
                    {(l.hist || []).length > 0 && (
                      <Tag color={T.purple}>
                        {l.hist.length} nota{l.hist.length !== 1 ? 's' : ''}
                      </Tag>
                    )}
                    {(l.docs || []).length > 0 && (
                      <Tag color={T.green}>
                        {l.docs.length} doc{l.docs.length !== 1 ? 's' : ''}
                      </Tag>
                    )}
                  </div>
                </div>
                <span
                  style={{ color: T.textDim, fontSize: 16, marginLeft: 10 }}
                >
                  →
                </span>
              </div>
...
function ToolObjecoes() {
  ...
  const obTxt =
    obId === 'outro'
      ? custom
      : OBJECOES.find((o) => o.id === obId)?.label || '';

  const gerar = async () => {
    if (!obId) {
      setErr('Selecione uma objeção.');
      return;
    }
    if (obId === 'outro' && !custom.trim()) {
      setErr('Digite a objeção.');
      return;
    }
    setErr('');
    setLoading(true);
  ...
  return (
    <div>
      <Title sub="O cliente travou? Selecione a objeção e receba a resposta ideal — empática, jurídica e dentro do EOAB.">
        🛡️ Quebrador de Objeções
      </Title>
      {!res ? (
        <>
          ...
          >
            {OBJECOES.map((ob) => (
              <button
                key={ob.id}
                onClick={() => setObId(ob.id)}
                style={{
                  background: obId === ob.id ? T.goldDim : T.card,
                  border: `1px solid ${obId === ob.id ? T.gold : T.border}`,
                  borderRadius: 9,
                  padding: '11px 13px',
                  color: obId === ob.id ? T.gold : T.textMuted,
                  fontSize: 12,
                  cursor: 'pointer',
                  textAlign: 'left',
                  fontFamily: 'Georgia,serif',
          ...
          <textarea
            ...
            rows={3}
            placeholder="Ex: cliente demitido após 5 anos, tem medo de represália do ex-patrão que é empresário conhecido na cidade..."
            style={{
              ...inp,
              resize: 'none',
              lineHeight: 1.6,
          ...
          <Btn
            ...
            style={{ width: '100%', padding: 14 }}
          >
            {loading ? '⏳ Gerando resposta...' : '→ Quebrar Objeção'}
          </Btn>
        </>
      ) : (
        <>
          ...
          <Card accent={T.green}>
            ...
            >
              <Tag color={T.green}>WHATSAPP</Tag>
              <CopyBtn text={res.resposta_whatsapp} />
            </div>
            <p
              style={{
                color: T.text,
          ...
          <Card accent={T.purple}>
            ...
            >
              <Tag color={T.purple}>PRESENCIAL / LIGAÇÃO</Tag>
              <CopyBtn text={res.resposta_presencial} />
            </div>
            <p
              style={{
                color: T.text,
...
function ToolCalc() {
  ...
  const calcular = () => {
    const sal = n(f.sal),
      meses = n(f.meses),
      dias = n(f.dias_ultimo_mes),
      fgts = n(f.fgts);
    if (sal <= 0 || meses <= 0) return;
    const items = [];
    let total = 0;
    const add = (nome, valor, desc, lei) => {
      items.push({ nome, valor, desc, lei });
      total += valor;
    };
    ...
    add(
      'Saldo de Salário',
      saldo,
      `${dias} dias trabalhados no último mês`,
      'Art. 477 CLT'
    );

    // 13º proporcional — art. 7º VIII CF/88
    const mesesRef = meses % 12 || 12;
    const avosRef = Math.ceil(meses % 12 === 0 ? 12 : meses % 12);
    const dec = (sal / 12) * avosRef;
    add(
      '13º Salário Proporcional',
      dec,
      `${avosRef}/12 avos`,
      'Art. 7º VIII CF/88 + Lei 4.090/62'
    );

    // Férias proporcionais + 1/3 — arts. 129 e 146 CLT
    const fer = (sal / 12) * avosRef;
    const ferTotal = fer * (4 / 3);
    add(
      'Férias Proporcionais + 1/3',
      ferTotal,
      `${avosRef}/12 avos + 1/3 constitucional`,
      'Arts. 129 e 146 CLT + art. 7º XVII CF/88'
    );

    if (f.tipo === 'sjc') {
      // Aviso prévio proporcional — Lei 12.506/11 + Súmula 441 TST
      if (f.aviso === 'indenizado') {
        const anosCompletos = Math.floor(meses / 12);
        const diasAviso = Math.min(30 + anosCompletos * 3, 90); // máx 90 dias
        const aviso = (sal / 30) * diasAviso;
        add(
          'Aviso Prévio Indenizado',
          aviso,
          `${diasAviso} dias (30 base + ${anosCompletos * 3} proporcionais)`,
          'Lei 12.506/11 + Súmula 441 TST'
        );
      }

      // FGTS multa 40% — art. 18 §1º Lei 8.036/90
      const fgtsSaldo = fgts > 0 ? fgts : sal * 0.08 * meses;
      const multa40 = fgtsSaldo * 0.4;
      add(
        'Multa FGTS 40%',
        multa40,
        `40% sobre R$ ${fgtsSaldo.toLocaleString('pt-BR', {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        })} ${fgts > 0 ? '(saldo informado)' : '(saldo estimado)'}`,
        'Art. 18 §1º Lei 8.036/90'
      );
    }

    if (f.tipo === 'ri') {
      // Rescisão indireta — art. 483 CLT — mesmas verbas da sem justa causa
      const anosCompletos = Math.floor(meses / 12);
      const diasAviso = Math.min(30 + anosCompletos * 3, 90);
      const aviso = (sal / 30) * diasAviso;
      add(
        'Aviso Prévio (Rescisão Indireta)',
        aviso,
        `${diasAviso} dias — culpa do empregador`,
        'Art. 483 CLT + Lei 12.506/11'
      );
      const fgtsSaldo = fgts > 0 ? fgts : sal * 0.08 * meses;
      add(
        'Multa FGTS 40%',
        fgtsSaldo * 0.4,
        'Rescisão indireta = mesmas verbas da SJC',
        'Art. 18 §1º Lei 8.036/90 + Súmula 246 TST'
      );
    }
    ...
    setRes({
      items,
      total,
      fgts_base: fgts > 0 ? fgts : n(f.sal) * 0.08 * meses,
    });
  };
  ...
  const verificarAI = async () => {
    ...
    try {
      const ctx = `Salário: R$${f.sal}\nMeses: ${f.meses}\nTipo: ${
        f.tipo
      }\nVerbas calculadas: ${res.items
        .map((i) => i.nome + ': R$' + i.valor.toFixed(2))
        .join(', ')}`;
  ...
  return (
    <div>
      <Title sub="Estimativa das verbas rescisórias com fundamentos legais. Base: CLT, CF/88 e Lei 12.506/11.">
      ...
      <Card>
        ...
        >
          <div>
            ...
            <input
              value={f.sal}
              onChange={(e) => up('sal', e.target.value)}
              placeholder="ex: 3500"
              type="number"
              style={inp}
            />
          ...
          <div>
            <Lbl>Tempo de empresa (meses)</Lbl>
            <input
              value={f.meses}
              onChange={(e) => up('meses', e.target.value)}
              placeholder="ex: 36"
              type="number"
              style={inp}
            />
          ...
          <div>
            <Lbl>Dias trabalhados no último mês</Lbl>
            <input
              value={f.dias_ultimo_mes}
              onChange={(e) => up('dias_ultimo_mes', e.target.value)}
              placeholder="1-30"
              type="number"
              style={inp}
            />
          ...
          <div>
            <Lbl>Saldo FGTS (R$) — opcional</Lbl>
            <input
              value={f.fgts}
              onChange={(e) => up('fgts', e.target.value)}
          ...
          <div style={{ gridColumn: '1/-1' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
              {tipos.map((t) => (
                <button
                  key={t.id}
                  onClick={() => up('tipo', t.id)}
                  style={{
                    background: f.tipo === t.id ? T.goldDim : 'transparent',
                    border: `1px solid ${f.tipo === t.id ? T.gold : T.border}`,
                    borderRadius: 7,
                    padding: '7px 12px',
                    color: f.tipo === t.id ? T.gold : T.textMuted,
                    fontSize: 12,
                    cursor: 'pointer',
                    transition: 'all 0.15s',
                  }}
              ...
              <div style={{ display: 'flex', gap: 7 }}>
                {[
                  ...
                  <button
                    ...
                    onClick={() => up('aviso', v)}
                    style={{
                      background: f.aviso === v ? T.goldDim : 'transparent',
                      border: `1px solid ${f.aviso === v ? T.gold : T.border}`,
                      borderRadius: 7,
                      padding: '7px 14px',
                      color: f.aviso === v ? T.gold : T.textMuted,
                      fontSize: 12,
                      cursor: 'pointer',
                    }}
                  >
        ...
        <Btn onClick={calcular} style={{ width: '100%', padding: 13 }}>
      ...
      {res && (
        <>
          ...
          >
            ...
            >
              R${' '}
              {res.total.toLocaleString('pt-BR', {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2,
              })}
            </span>
          ...
          >
            * Estimativa de referência. Valores reais dependem de acordos
            coletivos, horas extras e outras particularidades.
          </div>
          ...
          >
            {loadingAI
              ? '⏳ Verificando com IA...'
              : '🤖 IA: Verificar verbas esquecidas'}
          </Btn>

          {alerta && (
            <Card accent={T.purple}>
              <Lbl>🤖 Análise da IA — Verbas Adicionais</Lbl>
              {(alerta.verbas_extras || []).map((v, i) => (
                <div
                  key={i}
                  style={{ color: T.yellow, fontSize: 12, padding: '3px 0' }}
                >
                  → {v}
                </div>
              ))}
              {(alerta.alertas || []).map((a, i) => (
                <div
                  key={i}
                  style={{ color: T.red, fontSize: 12, padding: '3px 0' }}
                >
                  ⚠ {a}
                </div>
              ))}
...
function ToolContrato() {
  ...
  const gerar = async () => {
    if (!f.cliente || !f.caso) {
      setErr('Preencha nome do cliente e descrição do caso.');
      return;
    }
    setErr('');
    setLoading(true);
    try {
      const ctx = `CONTRATANTE: ${f.cliente}, CPF: ${f.cpf}, RG: ${
        f.rg
      }, Endereço: ${f.endereco}
ADVOGADO: ${f.advogado || '[Nome do Advogado]'}, OAB: ${
        f.oab || '[OAB/XX 000000]'
      }
...
HONORÁRIOS: ${
        f.tipo === 'exito'
          ? `${f.percentual || 30}% sobre o valor da condenação (êxito)`
          : f.tipo === 'fixo'
          ? `R$ ${f.honorarios} fixo`
          : f.tipo === 'misto'
          ? `R$ ${f.honorarios} entrada + ${f.percentual || 20}% êxito`
...
DATA: ${new Date().toLocaleDateString('pt-BR')}`;
  ...
  const campos = [
    { k: 'cliente', ph: 'Nome completo do cliente' },
    { k: 'cpf', ph: 'CPF: 000.000.000-00' },
    { k: 'rg', ph: 'RG: 00.000.000-0' },
    { k: 'advogado', ph: 'Dr(a). Nome Completo' },
    { k: 'oab', ph: 'OAB/XX 000000' },
    { k: 'cidade', ph: 'Cidade - UF' },
    { k: 'endereco', ph: 'Endereço completo do cliente', full: true },
    {
      k: 'caso',
      ph: 'Descreva as pretensões: ex: horas extras não pagas, FGTS, equiparação salarial...',
      full: true,
    },
  ];
  ...
  return (
    <div>
      <Title sub="Contrato completo e personalizado com base no Estatuto da OAB (Lei 8.906/94), EOAB, CLT e LGPD.">
          ...
          >
            ...
            <div>
              ...
              >
                ...
                <option value="misto">Misto (entrada + êxito)</option>
              </select>
            </div>
            <div>
              <Lbl>{f.tipo === 'fixo' ? 'Valor (R$)' : 'Percentual (%)'}</Lbl>
              <input
                value={f.tipo === 'fixo' ? f.honorarios : f.percentual}
                onChange={(e) =>
                  up(
                    f.tipo === 'fixo' ? 'honorarios' : 'percentual',
                    e.target.value
                  )
                }
                placeholder={f.tipo === 'fixo' ? 'ex: 2000' : 'ex: 30'}
                type="number"
                style={inp}
              />
            </div>
...
const TEMAS = [
  ...
  'FGTS não depositado',
  'Demissão durante doença/acidente',
...
function ToolPresenca() {
  ...
  const gerarConteudo = async () => {
    ...
    if (!t) {
      setErr('Escolha um tema.');
      return;
    }
    setErr('');
    setLoading(true);
    try {
      const ctx = `Canal: ${canal}\nTema jurídico: ${t}\nCidade do advogado: ${
        cidade || 'não informada'
      }\nGere post educativo e informativo sobre direito do trabalho.`;
      const r = await callClaude(P_CONTEUDO, ctx, 1000);
  ...
  const gerarAnuncio = async () => {
    ...
    setLoading(true);
    try {
      const ctx = `Plataforma: ${plataforma}\nCidade: ${cidadeAnuncio}\nOrçamento mensal: ${
        orcamento ? 'R$' + orcamento : 'não informado'
      }\nAdvogado trabalhista.`;
      const r = await callClaude(P_ANUNCIO, ctx, 800);
  ...
  const gerarIndicacao = async () => {
    if (!nomeEx) {
      setErr('Informe o nome do ex-cliente.');
      return;
    }
    setErr('');
    setLoading(true);
    try {
      const ctx = `Ex-cliente: ${nomeEx}\nCaso atendido: ${
        casoEx || 'não informado'
      }`;
      const r = await callClaude(P_INDICACAO, ctx, 800);
  ...
  const mods = [
    {
      ...
      icon: '✍️',
      label: 'Conteúdo',
      desc: 'Post educativo para redes sociais',
    },
    {
      id: 'anuncio',
      icon: '📢',
      label: 'Anúncio',
      desc: 'Google Ads ou Meta Ads',
    },
    {
      id: 'indicacao',
      icon: '🤝',
      label: 'Indicação',
      desc: 'Reativar ex-clientes',
    },
  ];

  return (
    <div>
      <Title sub="Apareça onde seu cliente está. Conteúdo educativo, anúncio e indicação — tudo dentro do EOAB.">
        📣 Presença Digital
      </Title>
      <div
        style={{
      ...
      >
        {mods.map((m) => (
          <button
            ...
            style={{
              background: mod === m.id ? T.goldDim : T.card,
              border: `1px solid ${mod === m.id ? T.gold : T.border}`,
              borderRadius: 11,
              padding: '13px 8px',
              cursor: 'pointer',
              textAlign: 'center',
      ...
      {mod === 'conteudo' && !res && (
        <Card>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
            {[
              ...
              <button
                ...
                style={{
                  flex: 1,
                  background: canal === v ? T.goldDim : 'transparent',
                  border: `1px solid ${canal === v ? T.gold : T.border}`,
                  borderRadius: 7,
                  padding: '8px 6px',
                  color: canal === v ? T.gold : T.textMuted,
                  fontSize: 11,
                  cursor: 'pointer',
                }}
              >
          ...
          >
            {TEMAS.map((t) => (
              <button
                ...
                onClick={() => setTema(t)}
                style={{
                  background: tema === t ? T.goldDim : T.surface,
                  border: `1px solid ${tema === t ? T.gold : T.border}`,
                  borderRadius: 7,
                  padding: '6px 11px',
                  color: tema === t ? T.gold : T.textMuted,
                  fontSize: 11,
                  cursor: 'pointer',
          ...
          <Btn
            ...
            style={{ width: '100%', padding: 13 }}
          >
            {loading ? '⏳ Gerando conteúdo...' : '→ Gerar Post Pronto'}
          </Btn>
        </Card>
      )}
        ...
        <Card>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
            {[
              ...
              <button
                ...
                style={{
                  flex: 1,
                  background: plataforma === v ? T.goldDim : 'transparent',
                  border: `1px solid ${plataforma === v ? T.gold : T.border}`,
                  borderRadius: 7,
                  padding: '9px',
                  color: plataforma === v ? T.gold : T.textMuted,
                  fontSize: 12,
                  cursor: 'pointer',
                }}
              >
          ...
          >
            <div>
              ...
              <input
                value={cidadeAnuncio}
                onChange={(e) => setCidadeAnuncio(e.target.value)}
                placeholder="ex: Curitiba"
                style={inp}
              />
            </div>
            <div>
              <Lbl>Orçamento/mês (R$) — opcional</Lbl>
              <input
                value={orcamento}
          ...
          <Btn
            ...
            style={{ width: '100%', padding: 13 }}
          >
            {loading ? '⏳ Gerando anúncio...' : '→ Gerar Texto de Anúncio'}
          </Btn>
        </Card>
      )}
      ...
      {mod === 'indicacao' && !res && (
        <Card>
          <InfoBox>
            💡 A melhor fonte de novos clientes é quem você já atendeu bem. Esta
            mensagem pede indicação de forma natural, sem constranger e dentro
            do EOAB.
          </InfoBox>
          ...
          <input
            value={casoEx}
            onChange={(e) => setCasoEx(e.target.value)}
            placeholder="ex: ganhou horas extras e FGTS após 5 anos"
            style={{ ...inp, marginBottom: 14 }}
          />
          <Err msg={err} />
          <Btn
            onClick={gerarIndicacao}
            disabled={loading}
            style={{ width: '100%', padding: 13 }}
          >
            {loading
              ? '⏳ Gerando mensagem...'
              : '→ Gerar Mensagem de Indicação'}
          </Btn>
        </Card>
      )}
      ...
      {res?.tipo === 'conteudo' && (
        <>
          ...
          <Btn variant="ghost" onClick={reset} style={{ width: '100%' }}>
            + Gerar outro
          </Btn>
        </>
      )}
      ...
      {res?.tipo === 'anuncio' && (
        <>
            ...
            {plataforma === 'google' ? (
              <>
                <div style={{ marginBottom: 8 }}>
                  <Lbl>Título (30 chars)</Lbl>
                  <div style={{ color: T.gold, fontSize: 15, fontWeight: 700 }}>
                    {res.titulo_principal}
                  </div>
                </div>
                <div>
                  <Lbl>Descrição (90 chars)</Lbl>
                  <div style={{ color: T.text, fontSize: 13 }}>
                    {res.descricao}
                  </div>
                </div>
          ...
          <Card>
            ...
            >
              {(res.palavras_chave || []).map((p, i) => (
                <Tag key={i} color={T.textMuted} bg={T.surface}>
                  {p}
                </Tag>
              ))}
            </div>
            ...
            <p style={{ color: T.yellow, fontSize: 13, margin: 0 }}>
      ...
      {res?.tipo === 'indicacao' && (
        <>
          <Card accent={T.green}>
            ...
            >
              <Tag color={T.green}>WHATSAPP</Tag>
              <CopyBtn text={res.mensagem_whatsapp} />
            </div>
            <p
              style={{
                color: T.text,
          ...
          <Card accent={T.purple}>
            ...
            >
              <Tag color={T.purple}>E-MAIL / LINKEDIN</Tag>
              <CopyBtn text={res.mensagem_email} />
            </div>
            <p
              style={{
                color: T.text,
      ...
      >
        <div
          style={{
            width: 32,
            height: 32,
            background: `linear-gradient(135deg,${T.gold},#7a5810)`,
            borderRadius: 8,
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
        ...
        <div style={{ flex: 1 }}>
          ...
          >
            SUÍTE JURÍDICA TRABALHISTA · PRODUTO FINAL
          </div>
        </div>
      </div>
          ...
          <button
            ...
            style={{
              ...
              borderBottom: `2px solid ${
                active === t.id ? T.gold : 'transparent'
              }`,
              padding: '10px 12px',
              color: active === t.id ? T.gold : T.textMuted,
              fontSize: 11,
              cursor: 'pointer',
              fontFamily: 'monospace',
              whiteSpace: 'nowrap',
